...
def get_audio_files(braille_codes, start_index):
	...
	for i, code in enumerate(braille_codes):
		...
		for (j, p) in enumerate(code):
			...
			if p == "1":
				# tone file
				copyfile("pure_tones/{0}.wav".format(INTERSPERSED_NOTES[NOTES_INDEX]), filename)
				NOTES_INDEX += 1
			else:
				# silent file
...
audio_index = 1
BRAILLE_CLUE_WORDS = BRAILLE_CLUEPHRASE.split(" ")
for (i, word) in enumerate(BRAILLE_CLUE_WORDS):
	audio_files = get_audio_files(map(lambda x:BRAILLE_DICT[x]["bin"], word), audio_index)
	...
	with open(OUTPUT_HTML, "w") as f:
		f.write(contents.replace(">>content", html_string + ("<br/>\n>>content" if not last_write else "")))
		f.close()

assert NOTES_INDEX == len(INTERSPERSED_NOTES)
